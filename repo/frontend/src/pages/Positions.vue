  ...
  <div>
    <div class="card">
      <div class="card-header">
        Positions
        <button v-if="canEdit" class="btn btn-primary btn-sm" @click="showCreate = true">Add Position</button>
      </div>
      <div class="card-body">
        <input v-model="search" class="form-control" placeholder="Search positions..." style="max-width: 300px; margin-bottom: 1rem;" @input="debouncedFetch" />
        <div v-if="loading" class="loading"><span class="spinner"></span> Loading...</div>
        <div v-else class="table-wrapper">
          <table>
            <thead><tr><th>Code</th><th>Title</th><th>Department</th><th>Status</th></tr></thead>
            <tbody>
    ...
    <div v-if="showCreate" style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000;">
      <div class="card" style="width: 500px;">
        <div class="card-header">Add Position <button class="btn btn-sm btn-secondary" @click="showCreate = false">Close</button></div>
        <div class="card-body">
          <div v-if="formError" class="alert alert-error">{{ formError }}</div>
          <form @submit.prevent="handleCreate">
            <div class="form-group"><label>Code</label><input v-model="form.code" class="form-control" required /></div>
            <div class="form-group"><label>Title</label><input v-model="form.title" class="form-control" required /></div>
            <div class="form-group"><label>Department</label><input v-model="form.department" class="form-control" /></div>
...
const form = reactive({ code: '', title: '', department: '', description: '' });
...
const handleCreate = async () => {
  ...
  try {
    ...
    Object.assign(form, { code: '', title: '', department: '', description: '' });
    fetchData();
